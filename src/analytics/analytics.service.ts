...
export class AnalyticsService {
  ...
  async getDashboardStats() {
    ...
    const clothsThisWeek = await this.productModel.countDocuments({ createdAt: { $gte: sevenDaysAgo } });

    // 2. Active Bookings (Booked + Rented)
    const activeBookings = await this.bookingModel.countDocuments({ status: { $in: ['booked', 'rented'] } });
    ...
    const pendingPaymentsData = await this.bookingModel.aggregate([
      ...
      { $group: { _id: null, total: { $sum: '$remainingPayment' }, count: { $sum: 1 } } }
    ]);
    ...
    const lastMonthRevenueData = await this.bookingModel.aggregate([
      { $match: { bookingDate: { $gte: startOfLastMonth, $lte: endOfLastMonth } } },
      {
        $group: {
          _id: null,
          total: {
    ...
    return {
      totalCloths: {
        value: totalCloths,
        trend: clothsThisWeek > 0 ? `+${clothsThisWeek} this week` : 'No new cloths this week'
      },
      activeBookings: {
        value: activeBookings,
        trend: activeSinceYesterday > 0 ? `+${activeSinceYesterday} since yesterday` : 'No new active bookings'
      },
      pendingPayments: {
      ...
      todaysReturns: {
        ...
        trend: `${todaysPendingReturns} pending inspection`
      }
    };
  }
  ...
  async getBookingStats(startDate: Date, endDate: Date) {
    ...
    const bookingDateData = await this.bookingModel.aggregate([
      ...
      },
      {
        $group: {
          _id: {
            date: { $dateToString: { format: '%Y-%m-%d', date: '$bookingDate', timezone: '+05:30' } },
            status: '$status'
          },
          count: { $sum: 1 }
        }
    ...
    const returnDateData = await this.bookingModel.aggregate([
      ...
      },
      {
        $group: {
          _id: {
            date: { $dateToString: { format: '%Y-%m-%d', date: '$returnDate', timezone: '+05:30' } },
            status: '$status'
          },
          count: { $sum: 1 }
        }
    ...
    const getOrInit = (date: string) => {
      if (!statsMap.has(date)) {
        statsMap.set(date, { booked: 0, rented: 0, pending_return: 0, returned: 0, cancelled: 0 });
      }
    ...
    while (current <= loopEnd) {
      ...
      const dayStats = statsMap.get(dateStr) || { booked: 0, rented: 0, pending_return: 0, returned: 0, cancelled: 0 };
      result.push({
        date: dateStr,
        ...dayStats
      });
